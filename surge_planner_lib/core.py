...
def run_planner(
    ...
    *,
    ...
    """
    High-level helper: build plan, apply multipliers, and optionally
    ...
    translated = apply_multipliers(plan, data.get("baselines", {}), days_cover=days_cover)
    human_brief = None
    if include_human_brief:
        human_brief = make_human_nlg(plan, features, env_blocks, translated, model=nlg_model)
    return {
        "plan": plan,
        "features": features,
